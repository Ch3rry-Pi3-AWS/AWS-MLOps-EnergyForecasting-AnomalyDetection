...
locals {
  ...
  source_dir             = abspath("${path.module}/../../sagemaker/forecast_sarimax_training")
  source_bundle_key      = var.source_bundle_key != null ? var.source_bundle_key : "sagemaker/forecast_sarimax_training/source/forecast_sarimax_training_source.tar.gz"
  ...
  job_base_name          = var.job_base_name != null ? var.job_base_name : "${var.deployment_name}-forecast-sarimax-train"

  # Reuse the SageMaker scikit-learn framework image and install the pinned
  # statsmodels dependency set from `requirements.txt` inside the source bundle.
