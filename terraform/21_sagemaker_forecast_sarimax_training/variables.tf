variable "aws_region" {
  description = "AWS region in which SARIMAX forecast training assets should be staged."
  type        = string
}
...
variable "forecast_model_package_group_name" {
  description = "Name of the forecast model package group that trained SARIMAX model versions should be registered into."
  type        = string
}

variable "job_base_name" {
  description = "Optional deterministic base name used by the SARIMAX training runner before it appends a timestamp."
  type        = string
  default     = null
}

variable "source_bundle_key" {
  description = "Optional object key used when uploading the SARIMAX training source bundle to the artefact bucket."
  type        = string
  default     = null
}
...
variable "training_output_prefix" {
  description = "Artefact-bucket prefix under which SageMaker SARIMAX training outputs should be written."
  type        = string
  default     = "sagemaker/forecast_sarimax_training/output"
}
