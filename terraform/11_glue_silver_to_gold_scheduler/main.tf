...
locals {
  ...
  schedule_name = var.schedule_name != null ? var.schedule_name : "scheduler-silver-gold-${var.deployment_name}"
  role_name     = var.scheduler_role_name != null ? var.scheduler_role_name : "iam-scheduler-gold-${var.deployment_name}"

  # EventBridge Scheduler universal targets invoke an AWS API operation
  # directly. Here the schedule calls Glue StartJobRun with the deployed Gold
  # job name and any optional per-run arguments supplied by the caller.
...
# Glue StartJobRun on the Silver-to-Gold job.
...
# Silver-to-Gold Glue job rather than to Glue jobs in general.
data "aws_iam_policy_document" "scheduler_start_glue_job" {
  statement {
    sid       = "StartSilverToGoldJob"
