"""Deploy the latest approved forecast model package to a SageMaker endpoint.

This script consumes Terraform outputs from the forecast-endpoint module and
the earlier SageMaker registry stage. It resolves the latest approved model
package in the forecast registry, creates a concrete SageMaker model and
endpoint configuration, and then creates or updates the stable forecast
endpoint.
...
- The script expects at least one approved model package in the forecast
  registry. If none exist, approve a registered version first in Studio or via
  the SageMaker API.

Examples
--------
Deploy the latest approved forecast model and wait for the endpoint:

>>> # uv run python scripts/deploy_forecast_endpoint.py

Submit the create or update request and return immediately:

>>> # uv run python scripts/deploy_forecast_endpoint.py --no-wait
"""
...
def get_output(tf_dir: Path, output_name: str) -> str:
    ...
    if value is None:
        raise RuntimeError(
            ...
            "`python scripts\\deploy.py --forecast-endpoint-only` for the forecast-endpoint stage."
        )
    return value
...
def get_latest_approved_model_package_arn(sagemaker_client, model_package_group_name: str) -> str:
    """
    Resolve the newest approved model package in the forecast registry.
    ...
    model_package_group_name : str
        Forecast model package group name.
    ...
    if not packages:
        raise RuntimeError(
            "No approved forecast model packages were found. Approve a registered model version in Studio "
...
def main() -> None:
    """Deploy the latest approved forecast model package to the stable endpoint."""

    parser = argparse.ArgumentParser(
        description="Deploy the latest approved forecast model package to a SageMaker endpoint."
    )
    parser.add_argument(
        "--model-package-arn",
        default=None,
        help="Optional explicit model package ARN. If omitted, the latest approved forecast package is used.",
    )
    ...
    model_name_base = get_output(endpoint_dir, "forecast_endpoint_model_name_base")
    endpoint_config_name_base = get_output(endpoint_dir, "forecast_endpoint_config_name_base")
    instance_type = get_output(endpoint_dir, "forecast_endpoint_instance_type")
    initial_instance_count = int(get_output(endpoint_dir, "forecast_endpoint_initial_instance_count"))
    variant_name = get_output(endpoint_dir, "forecast_endpoint_variant_name")
    kms_key_arn = get_output(endpoint_dir, "forecast_endpoint_kms_key_arn")
    role_arn = get_output(endpoint_dir, "forecast_endpoint_role_arn")
    model_package_group_name = get_output(endpoint_dir, "forecast_model_package_group_name")
