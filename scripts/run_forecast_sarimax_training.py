"""Launch a SageMaker SARIMAX forecast-training job and register the result.

This script mirrors the baseline forecast-training runner but packages a
genuinely sequential SARIMAX forecasting implementation instead of the
existing tabular gradient-boosting baseline.
...
- The script packages the local SARIMAX training code as a `.tar.gz`, uploads
  it to the artefact bucket, starts the SageMaker training job, and optionally
  registers the resulting model package into the existing forecast registry.

Examples
--------
Run the full SARIMAX training-and-registration flow:

>>> # uv run python scripts/run_forecast_sarimax_training.py

Submit only the training job and return immediately:

>>> # uv run python scripts/run_forecast_sarimax_training.py --no-wait
"""
...
try:
    ...
    raise SystemExit(
        "boto3 is required to run SARIMAX forecast training locally. Install project dependencies with "
        "`uv sync --dev` and then rerun via `uv run python scripts/run_forecast_sarimax_training.py`."
    ) from exc
...
def get_output(tf_dir: Path, output_name: str) -> str:
    ...
    if value is None:
        raise RuntimeError(
            ...
            "`python scripts\\deploy.py --forecast-sarimax-training-only` for the SARIMAX training stage."
        )
    return value
...
def register_model_package(
    ...
    *,
    ...
    """Register the completed SARIMAX training artefact into SageMaker Model Registry."""

    response = sagemaker_client.create_model_package(
        ModelPackageGroupName=model_package_group_name,
        ModelApprovalStatus="PendingManualApproval",
        ModelPackageDescription=(
            f"SARIMAX forecast model trained from Gold forecast features for {deployment_name}."
        ),
...
def main() -> None:
    """Launch the SARIMAX forecast training job and optionally register the result."""

    parser = argparse.ArgumentParser(
        description="Launch a SageMaker SARIMAX forecast-training job and register the resulting model package."
    )
    ...
    training_dir = REPO_ROOT / "terraform" / "21_sagemaker_forecast_sarimax_training"
    context_dir = REPO_ROOT / "terraform" / "01_project_context"

    region = get_output(training_dir, "forecast_sarimax_training_region")
    deployment_name = get_output(context_dir, "deployment_name")
    role_arn = get_output(training_dir, "forecast_sarimax_training_role_arn")
    source_bundle_s3_uri = get_output(training_dir, "forecast_sarimax_training_source_bundle_s3_uri")
    source_bundle_key = get_output(training_dir, "forecast_sarimax_training_source_bundle_key")
    source_dir = Path(get_output(training_dir, "forecast_sarimax_training_source_dir"))
    input_s3_uri = get_output(training_dir, "forecast_sarimax_training_input_s3_uri")
    output_s3_uri = get_output(training_dir, "forecast_sarimax_training_output_s3_uri")
    training_image_uri = get_output(training_dir, "forecast_sarimax_training_image_uri")
    inference_image_uri = get_output(training_dir, "forecast_sarimax_inference_image_uri")
    model_package_group_name = get_output(training_dir, "forecast_sarimax_model_package_group_name")
    job_base_name = get_output(training_dir, "forecast_sarimax_training_job_base_name")
    kms_key_arn = get_output(training_dir, "forecast_sarimax_training_kms_key_arn")
