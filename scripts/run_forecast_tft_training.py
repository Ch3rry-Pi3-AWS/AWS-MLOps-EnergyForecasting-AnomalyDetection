"""Launch a SageMaker TFT forecast-training job and register the result.

This script mirrors the existing custom forecast-training runners but packages
and launches a Temporal Fusion Transformer implementation inside a SageMaker
PyTorch container instead of using the scikit-learn or built-in DeepAR paths.

Examples
--------
Run the full TFT training-and-registration flow:

>>> # uv run python scripts/run_forecast_tft_training.py

Submit only the training job and return immediately:

>>> # uv run python scripts/run_forecast_tft_training.py --no-wait
"""
...
try:
    ...
    raise SystemExit(
        "boto3 is required to run TFT forecast training locally. Install project dependencies with "
        "`uv sync --dev` and then rerun via `uv run python scripts/run_forecast_tft_training.py`."
    ) from exc
...
def get_output(tf_dir: Path, output_name: str) -> str:
    ...
    if value is None:
        raise RuntimeError(
            ...
            "`python scripts\\deploy.py --forecast-tft-training-only` for the TFT training stage."
        )
    return value
...
def register_model_package(
    ...
    *,
    ...
    """Register the completed TFT training artefact into SageMaker Model Registry."""

    response = sagemaker_client.create_model_package(
        ModelPackageGroupName=model_package_group_name,
        ModelApprovalStatus="PendingManualApproval",
        ModelPackageDescription=(
            f"Temporal Fusion Transformer forecast model trained from Gold forecast features for {deployment_name}."
        ),
...
def main() -> None:
    """Launch the TFT forecast training job and optionally register the result."""

    parser = argparse.ArgumentParser(
        description="Launch a SageMaker TFT forecast-training job and register the resulting model package."
    )
    ...
    parser.add_argument("--volume-size-gb", type=int, default=50, help="Training volume size in GB.")
    ...
    training_dir = REPO_ROOT / "terraform" / "23_sagemaker_forecast_tft_training"
    context_dir = REPO_ROOT / "terraform" / "01_project_context"

    region = get_output(training_dir, "forecast_tft_training_region")
    deployment_name = get_output(context_dir, "deployment_name")
    role_arn = get_output(training_dir, "forecast_tft_training_role_arn")
    source_bundle_s3_uri = get_output(training_dir, "forecast_tft_training_source_bundle_s3_uri")
    source_bundle_key = get_output(training_dir, "forecast_tft_training_source_bundle_key")
    source_dir = Path(get_output(training_dir, "forecast_tft_training_source_dir"))
    input_s3_uri = get_output(training_dir, "forecast_tft_training_input_s3_uri")
    output_s3_uri = get_output(training_dir, "forecast_tft_training_output_s3_uri")
    training_image_uri = get_output(training_dir, "forecast_tft_training_image_uri")
    inference_image_uri = get_output(training_dir, "forecast_tft_inference_image_uri")
    model_package_group_name = get_output(training_dir, "forecast_tft_model_package_group_name")
    job_base_name = get_output(training_dir, "forecast_tft_training_job_base_name")
    kms_key_arn = get_output(training_dir, "forecast_tft_training_kms_key_arn")

    hyperparameters = {
        "context_length": get_output(training_dir, "forecast_tft_context_length"),
        "prediction_length": get_output(training_dir, "forecast_tft_prediction_length"),
        "max_epochs": get_output(training_dir, "forecast_tft_max_epochs"),
        "batch_size": get_output(training_dir, "forecast_tft_batch_size"),
        "hidden_size": get_output(training_dir, "forecast_tft_hidden_size"),
        "attention_head_size": get_output(training_dir, "forecast_tft_attention_head_size"),
        "hidden_continuous_size": get_output(training_dir, "forecast_tft_hidden_continuous_size"),
        "dropout": get_output(training_dir, "forecast_tft_dropout"),
        "learning_rate": get_output(training_dir, "forecast_tft_learning_rate"),
    }
