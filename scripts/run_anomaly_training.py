"""Launch a SageMaker anomaly-training job and register the result.

This script consumes Terraform outputs from the anomaly-training module and
earlier infrastructure stages. It starts an ephemeral SageMaker training job,
waits for completion by default, and then registers the produced model
artefacts into the anomaly model package group created earlier.
...
- This script packages the local anomaly-training code as a `.tar.gz`,
  uploads it to the artefact bucket, starts the SageMaker training job, and
  optionally registers the resulting model package.
- The first implementation deliberately trains a simple baseline anomaly
  detector so the project can complete an end-to-end SageMaker registration
  flow for both model families.

Examples
--------
Run the full anomaly-training-and-registration flow:

>>> # uv run python scripts/run_anomaly_training.py

Submit only the anomaly training job and return immediately:

>>> # uv run python scripts/run_anomaly_training.py --no-wait
"""
...
try:
    ...
    raise SystemExit(
        "boto3 is required to run anomaly training locally. Install project dependencies with "
        "`uv sync --dev` and then rerun via `uv run python scripts/run_anomaly_training.py`."
    ) from exc
...
def get_output(tf_dir: Path, output_name: str) -> str:
    ...
    if value is None:
        raise RuntimeError(
            ...
            "`python scripts\\deploy.py --anomaly-training-only` for the anomaly-training stage."
        )
    return value
...
def wait_for_training_completion(
    ...
    """Poll SageMaker until the anomaly training job reaches a terminal state."""
...
def register_model_package(
    ...
    *,
    ...
    """Register the completed anomaly model artefact into SageMaker Model Registry."""

    response = sagemaker_client.create_model_package(
        ModelPackageGroupName=model_package_group_name,
        ModelApprovalStatus="PendingManualApproval",
        ModelPackageDescription=(
            f"Baseline anomaly model trained from Gold anomaly features for {deployment_name}."
        ),
...
def main() -> None:
    """Launch the anomaly training job and optionally register the result."""

    parser = argparse.ArgumentParser(
        description="Launch a SageMaker anomaly-training job and register the resulting model package."
    )
    ...
    region = get_output(training_dir, "anomaly_training_region")
    deployment_name = get_output(context_dir, "deployment_name")
    role_arn = get_output(training_dir, "anomaly_training_role_arn")
    source_bundle_s3_uri = get_output(training_dir, "anomaly_training_source_bundle_s3_uri")
    source_bundle_key = get_output(training_dir, "anomaly_training_source_bundle_key")
    source_dir = Path(get_output(training_dir, "anomaly_training_source_dir"))
    input_s3_uri = get_output(training_dir, "anomaly_training_input_s3_uri")
    output_s3_uri = get_output(training_dir, "anomaly_training_output_s3_uri")
    training_image_uri = get_output(training_dir, "anomaly_training_image_uri")
    inference_image_uri = get_output(training_dir, "anomaly_inference_image_uri")
    model_package_group_name = get_output(training_dir, "anomaly_model_package_group_name")
    job_base_name = get_output(training_dir, "anomaly_training_job_base_name")
    kms_key_arn = get_output(training_dir, "anomaly_training_kms_key_arn")
