"""Inference entry point for the baseline anomaly model package.

This script is bundled into the registered SageMaker model package so the
first anomaly model version can be deployed later without rewriting inference
logic.
...
def model_fn(model_dir: str) -> dict[str, Any]:
    """Load the persisted anomaly model bundle from SageMaker model storage."""
...
def predict_fn(input_data: pd.DataFrame, model_bundle: dict[str, Any]) -> list[dict[str, float | bool]]:
    """Run anomaly inference and return both scores and boolean flags."""

    feature_columns = model_bundle["feature_columns"]
    aligned = input_data.reindex(columns=feature_columns, fill_value=0.0)
    scores = model_bundle["model"].score_samples(aligned)
