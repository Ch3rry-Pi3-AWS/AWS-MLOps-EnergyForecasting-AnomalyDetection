"""Inference entry point for the baseline forecast model package.

This script is bundled into the registered SageMaker model package so the
initial model version can be deployed later without rewriting inference logic.
...
def model_fn(model_dir: str) -> dict[str, Any]:
    """Load the persisted model bundle from SageMaker model storage."""
...
def predict_fn(input_data: pd.DataFrame, model_bundle: dict[str, Any]) -> list[float]:
    """Run model inference after aligning the incoming columns to training order."""
